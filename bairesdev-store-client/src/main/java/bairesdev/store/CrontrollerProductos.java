...
public class CrontrollerProductos {
	...
	public String index(@ModelAttribute("model") ModelMap model) {
		...
		try {
			...
			ResponseEntity<ProductoList> result1 = restTemplate.exchange(
					"http://localhost:8080/bairesdev-store-server/store/productos", 
			...
			ResponseEntity<CategoriaList> result2 = restTemplate.exchange(
					"http://localhost:8080/bairesdev-store-server/store/categorias", 
			...
			ResponseEntity<MedidaList> result3 = restTemplate.exchange(
					"http://localhost:8080/bairesdev-store-server/store/medidas", 
		...
		if (producto != null &&  
			...
			try {
				ResponseEntity<Producto> result = restTemplate.exchange(
						"http://localhost:8080/bairesdev-store-server/store/productos/incluir", 
	...
	public String modificar(@ModelAttribute("producto") Producto producto, @RequestParam(value="idString",  required=true) String idString) {
		if (producto != null &&  
			...
			try {
				ResponseEntity<Producto> result = restTemplate.exchange(
						"http://localhost:8080/bairesdev-store-server/store/productos/modificar", 
	...
	public String eliminar(@PathVariable Integer id) {
		...
		ResponseEntity<String> result = restTemplate.exchange(
								"http://localhost:8080/bairesdev-store-server/store/productos/eliminar/"+id, 
